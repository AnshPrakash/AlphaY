...
class YinshBot_v2 {
	...
	public static int Discindiag(String[][] curr_board,int[] a1, int[] a2){
		...
		if(a1[0]+a1[1]==a2[0]+a2[1]){
			...
			diag2_score = 121*n[4]+40*n[3]+ 13*n[2]+ 4*n[1]+n[0];
		}
		else if(a1[0]-a1[1]==a2[0]-a2[1]){
			int i=a1[0];
			int j=a1[1];
		...
		// return diag1_score;
		if (my_player_no==1) {
			return(diag1_score- diag2_score);
		}

		return diag2_score- diag1_score;
	}    
	public static int Discinrowver(String[][] curr_board){
		...
		ver2_score = 121*n[4]+40*n[3]+ 13*n[2]+ 4*n[1]+n[0];
	
		if (my_player_no==1) {
			return(ver_score- ver2_score);
		}
		
		
		return (ver2_score- ver_score);
	}
	...
	public static int Utility(String[][] curr_board) {
		...
		util = 100*util1+35*cring1+25*seq1+5000*less_rings+300*whatif;
	...
	public static int PVS(String[][] curr_board, int player_no, int depth, int color, int alpha, int beta,int cutoff){
		...
		for (int i = 0; i < neighbours.size(); i++) {
			...
			if (i==0) {
				...
				score= -NegaMax(neighbours.elementAt(i).getFirst(), temp_p, depth+1,-color,-alpha-margin,-alpha,cutoff);
...
class Pair<F, S> implements Comparable<Pair<String[][], String>> {
    ...
    public int compareTo(Pair<String[][],String> p) {
    	int c1=YinshBot_v2.Utility(p.getFirst());
    	int c2=YinshBot_v2.Utility(((Pair<String[][],String>)this).getFirst());
        return (c2-c1);
    }
}